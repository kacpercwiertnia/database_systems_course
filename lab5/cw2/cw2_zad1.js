...
db.getCollection('tip')
    .aggregate([
        ...
        {$project: {
            _id: 0,
            name: {$arrayElemAt:["$business_info.name",0]},
            numOfTips: 1
        }}
    ])
...
db.getCollection('review')
    .aggregate([
        {$project:{
            _id: "$review_id",
            funny: { $cond: [{ $gt: [ "$votes.funny", 1 ]}, 1, 0 ]},
            useful: { $cond: [{ $gt: [ "$votes.useful", 1 ]}, 1, 0 ]},
            cool: { $cond: [{ $gt: [ "$votes.cool", 1 ]}, 1, 0 ]}
        }},
        {$group:{
            _id: "votes",
            numberOfFunny: { $sum: "$funny"},
            numberOfUseful: { $sum: "$useful"},
            numberOfCool: { $sum: "$cool"}
        }},
    ])
